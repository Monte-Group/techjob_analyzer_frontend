...
export type ChartPayload =
  ...
  | { type: "salaries"; data: Salary[] }
  | { type: "trends"; data: Trend[] }
  | { type: "regions"; data: RegionStat[] }
  | { type: "experience"; data: LabeledCount[] }
  | { type: "employment"; data: LabeledCount[] }
  | { type: "companies"; data: CompanyStat[] }
  | { type: "salary_histogram"; data: SalaryBucket[] }
  | { type: "skill_compare"; data: SkillCompareItem[] }
  | { type: "generic_bar"; data: { label: string; value: number }[] };

const chartColors = ["#7c6cff", "#1f8f78", "#3f6df6", "#e06b5f", "#8b5cf6", "#0f9db4"];
const tickStyle = { fill: "var(--text-dim)", fontSize: 12 };
const compactTickStyle = { fill: "var(--text-dim)", fontSize: 10 };
...
export function AIChatChart({
  ...
  if (chart.type === "skills") return <SkillsChart data={chart.data} compact={compact} />;
  if (chart.type === "salaries") return <SalariesChart data={chart.data} compact={compact} />;
  if (chart.type === "trends") return <TrendsChart data={chart.data} compact={compact} />;
  if (chart.type === "regions") return <RegionsChart data={chart.data} compact={compact} />;
  if (chart.type === "experience") return <MiniDistributionChart data={chart.data} tone="#1d4ed8" />;
  if (chart.type === "employment") return <MiniDistributionChart data={chart.data} tone="#dc2626" />;
  if (chart.type === "companies") return <CompaniesChart data={chart.data} compact={compact} />;
  if (chart.type === "salary_histogram") return <SalaryHistogramChart data={chart.data} compact={compact} />;
  if (chart.type === "skill_compare") return <SkillCompareChart data={chart.data} compact={compact} />;
  return <GenericBarChart data={chart.data} compact={compact} />;
}
...
export function SkillsChart({ data, compact = false }: { data: Skill[]; compact?: boolean }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={compact ? Math.max(300, data.length * 32) : 420}>
      <BarChart data={data} layout="vertical" margin={{ left: compact ? 24 : 60, right: compact ? 12 : 10 }}>
        <CartesianGrid stroke={gridStroke} horizontal={false} />
        <XAxis type="number" tick={compact ? compactTickStyle : tickStyle} axisLine={{ stroke: axisStroke }} tickLine={false} />
        <YAxis
          dataKey="skill"
          type="category"
          width={compact ? 116 : 100}
          tick={compact ? compactTickStyle : tickStyle}
          tickFormatter={(value: string) => compact ? compactLabel(value) : value}
...
export function SalariesChart({ data, compact = false }: { data: Salary[]; compact?: boolean }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={Math.max(compact ? 280 : 360, data.length * (compact ? 44 : 52))}>
      <BarChart data={data} layout="vertical" margin={{ left: compact ? 28 : 80, right: compact ? 12 : 16, top: compact ? 8 : 0 }}>
        <CartesianGrid stroke={gridStroke} horizontal={false} />
        <XAxis type="number" tick={compact ? compactTickStyle : tickStyle} axisLine={{ stroke: axisStroke }} tickLine={false} tickFormatter={(v) => `${Math.round(Number(v) / 1000)}k`} />
        <YAxis
          dataKey="skill"
          type="category"
  ...
  return (
    <ResponsiveContainer width="100%" height={compact ? 280 : 360}>
      <BarChart data={data}>
        ...
        />
        <YAxis tick={compact ? compactTickStyle : tickStyle} axisLine={{ stroke: axisStroke }} tickLine={false} />
        <Tooltip formatter={(value) => [fmtInt(Number(value ?? 0)), "Вакансий"]} />
...
export function SkillCompareChart({ data, compact = false }: { data: SkillCompareItem[]; compact?: boolean }) {
  ...
  return (
    <div className={compact ? "space-y-4" : "space-y-6"}>
      <ResponsiveContainer width="100%" height={compact ? 250 : 320}>
        <BarChart data={data}>
          <CartesianGrid stroke={gridStroke} strokeDasharray="3 3" />
          <XAxis
            dataKey="skill"
            tick={compact ? compactTickStyle : tickStyle}
            tickFormatter={(value: string) => compact ? compactLabel(value, 10) : value}
            axisLine={{ stroke: axisStroke }}
            tickLine={false}
            angle={compact ? -24 : 0}
            textAnchor={compact ? "end" : "middle"}
            height={compact ? 72 : 30}
          />
          <YAxis tick={compact ? compactTickStyle : tickStyle} axisLine={{ stroke: axisStroke }} tickLine={false} />
          <Tooltip formatter={(value) => [fmtInt(Number(value ?? 0)), "Вакансий"]} />
...
export function GenericBarChart({ data, compact = false }: { data: { label: string; value: number }[]; compact?: boolean }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={Math.max(compact ? 240 : 280, data.length * (compact ? 40 : 44))}>
      <BarChart data={data} layout="vertical" margin={{ left: compact ? 30 : 80, right: compact ? 12 : 16 }}>
        <CartesianGrid stroke={gridStroke} horizontal={false} />
        <XAxis type="number" tick={compact ? compactTickStyle : tickStyle} axisLine={{ stroke: axisStroke }} tickLine={false} />
        <YAxis
          dataKey="label"
          type="category"
          width={compact ? 128 : 120}
          tick={compact ? compactTickStyle : tickStyle}
          tickFormatter={(value: string) => compact ? compactLabel(value, 18) : value}
          axisLine={{ stroke: axisStroke }}
          tickLine={false}
          interval={0}
        />
...
export function TrendsChart({ data, compact = false }: { data: Trend[]; compact?: boolean }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={compact ? 260 : 360}>
      <LineChart data={data} margin={{ left: 8, right: 16, bottom: compact ? 18 : 0 }}>
        <CartesianGrid stroke={gridStroke} strokeDasharray="3 3" />
        <XAxis
          dataKey="month"
          tick={compact ? compactTickStyle : tickStyle}
...
export function RegionsChart({ data, compact = false }: { data: RegionStat[]; compact?: boolean }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={compact ? 280 : 420}>
      <BarChart data={data.slice(0, 10)} layout="vertical" margin={{ left: compact ? 28 : 95, right: compact ? 12 : 10 }}>
        <CartesianGrid stroke={gridStroke} horizontal={false} />
        <XAxis type="number" tick={compact ? compactTickStyle : tickStyle} axisLine={{ stroke: axisStroke }} tickLine={false} />
        <YAxis
          dataKey="region"
          type="category"
          width={compact ? 126 : 120}
          tick={compact ? compactTickStyle : tickStyle}
          tickFormatter={(value: string) => compact ? compactLabel(value, 16) : value}
          axisLine={{ stroke: axisStroke }}
          tickLine={false}
          interval={0}
        />
...
export function CompaniesChart({ data, compact = false }: { data: CompanyStat[]; compact?: boolean }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={compact ? 280 : 420}>
      <BarChart data={data.slice(0, 10)} layout="vertical" margin={{ left: compact ? 28 : 100, right: compact ? 12 : 10 }}>
        <CartesianGrid stroke={gridStroke} horizontal={false} />
        <XAxis type="number" tick={compact ? compactTickStyle : tickStyle} axisLine={{ stroke: axisStroke }} tickLine={false} />
        <YAxis
          dataKey="company"
          type="category"
          width={compact ? 134 : 135}
          tick={compact ? compactTickStyle : tickStyle}
          tickFormatter={(value: string) => compact ? compactLabel(value, 18) : value}
          axisLine={{ stroke: axisStroke }}
          tickLine={false}
